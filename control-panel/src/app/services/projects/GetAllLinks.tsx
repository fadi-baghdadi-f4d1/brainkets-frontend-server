...
export const getLinks = async (id: number) => {
  try {
    ...
    const response = await baseApi.get('/get-project-links', {
        headers: {
            Authorization: token,
          },
      params: {
    ...
    if (axios.isAxiosError(error)) {
      ...
      throw new Error(error.response?.data?.message || 'Get project links failed');
    } else {
      console.error('Non-Axios error:', error.message || 'Get project links failed');
